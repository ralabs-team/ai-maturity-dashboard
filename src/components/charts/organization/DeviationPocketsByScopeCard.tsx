...
import type { DeviationScopeRow } from '../../../data/survey/deviationInsights';
...
const QUADRANT_BADGES: QuadrantBadge[] = [
  {
    title: 'Peer-aligned',
    description: 'Mostly similar to project and department peers',
    className: 'left-3 bottom-3',
  },
  {
    title: 'Project mismatch',
    description: 'Different from day-to-day project peers',
    className: 'right-3 bottom-3 text-right',
  },
  {
    title: 'Department outlier',
    description: 'Different from the wider department pattern',
    className: 'left-3 top-3',
  },
  {
    title: 'Strong outlier pocket',
    description: 'Different from both project and department peers',
...
function TooltipContent({
  ...
  active?: boolean;
  payload?: Array<{ payload?: DeviationScopeRow }>;
  scope: Scope;
}) {
  ...
  return (
    <div className="rounded-md bg-[#242424] px-4 py-3 text-sm text-white shadow-lg">
      ...
      <div className="space-y-1">
        <div>Avg project-group deviation: {point.avgProjectDeviation.toFixed(1)}</div>
        <div>Avg department deviation: {point.avgDepartmentDeviation.toFixed(1)}</div>
        <div>Avg combined deviation: {point.avgCombinedDeviation.toFixed(1)}</div>
        <div>High-deviation share: {point.highDeviationShare.toFixed(1)}%</div>
        <div>Isolated high-deviation share: {point.isolatedHighDeviationShare.toFixed(1)}%</div>
...
export default function DeviationPocketsByScopeCard({
  departmentRows,
  teamRows,
  scope: forcedScope,
  hideScopeToggle = false,
}: {
  departmentRows: DeviationScopeRow[];
  teamRows: DeviationScopeRow[];
  scope?: Scope;
  ...
  return (
    <section className="rounded-2xl border border-[#eaeaea] bg-white p-6 shadow-sm">
      <ChartFeedback chartTitle="Deviation pockets by scope" page="organization" />
      <h3 className="text-lg font-semibold tracking-tight text-[#242424]">
        Deviation pockets by scope
      </h3>
      <p className="mt-1 text-sm text-[#7a7a7a]">
        See which departments or teams contain people who look unusually different from both their
        project group and their department peers. Top-right scopes are the strongest outlier
        pockets.
      </p>
      ...
      {rows.length > 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 10, right: 12, left: 0, bottom: 18 }}>
              ...
              <XAxis
                ...
                tickLine={false}
                label={{
                  value: 'Average deviation vs project group',
                  position: 'bottom',
                  offset: 6,
                  fill: '#525252',
                  fontSize: 12,
              ...
              <YAxis
                type="number"
                dataKey="avgDepartmentDeviation"
                domain={[0, 2.5]}
                ticks={[0, 0.5, 1, 1.5, 2, 2.5]}
                tick={{ fontSize: 12, fill: '#737373' }}
                axisLine={false}
                tickLine={false}
                label={{
                  value: 'Average deviation vs department',
                  angle: -90,
                  position: 'insideLeft',
                  fill: '#525252',
                  fontSize: 12,
              ...
              <ReferenceLine x={0.75} stroke="#d4d4d8" strokeDasharray="4 4" />
              <ReferenceLine y={0.75} stroke="#d4d4d8" strokeDasharray="4 4" />
              <Scatter data={rows}>
                {rows.map((row) => (
        ...
        <div className="mt-6 flex h-[220px] items-center justify-center rounded-2xl border border-dashed border-[#d4d4d8] bg-[#fafafa] px-6 text-center text-sm text-[#7a7a7a]">
          No {scope === 'department' ? 'departments' : 'teams'} have enough data for this chart yet.
        </div>
      )}
    </section>
  );
